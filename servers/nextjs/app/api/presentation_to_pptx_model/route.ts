...
async function getBrowserAndPage(id: string): Promise<[Browser, Page]> {
  ...
  // Listen for console events from the page and log them to the server console
  ...
  console.log('Navigating to pdf-maker page...');
  await page.goto(`http://localhost:3000/pdf-maker?id=${id}`, {
    waitUntil: "networkidle0",
    timeout: 300000,
  });
  console.log('Navigation complete. Waiting for ready signal...');

  await page.waitForSelector('[data-is-ready="true"]', { timeout: 300000 });
  console.log('Ready signal received from pdf-maker page.');
  ...
  try {
    await page.evaluate(() => {
      ...
      return new Promise<void>((resolve) => {
        const checkReady = () => {
          const isReady = document.querySelector('[data-is-ready="true"]');
          if (isReady) {
            resolve();
          } else {
    ...
    console.error("Error while waiting for presentation to be ready:", error);
  }
...
function getScreenshotsDir() {
  ...
  if (!tempDir) {
    // If running locally without env var, create a default temp dir
    tempDir = path.join(process.cwd(), ".temp");
    console.log(`TEMP_DIRECTORY not set, using default: ${tempDir}`);
  }

...
const PresentationPage = ({ presentation_id }: { presentation_id: string }) => {
  ...
  useEffect(() => {
    console.log("PDF Maker: loading state changed", { loading, contentLoading });
    if (!loading && !contentLoading && presentationData?.slides && presentationData.slides.length > 0) {
      console.log("PDF Maker: Data is ready, setting data-is-ready=true");
    }
  ...
  const fetchUserSlides = async () => {
    try {
      console.log("PDF Maker: Fetching user slides...");
      const data = await DashboardApi.getPresentation(presentation_id);
      dispatch(setPresentationData(data));
      setContentLoading(false);
      console.log("PDF Maker: Successfully fetched user slides.");
    } catch (error) {
      setError(true);
  ...
  return (
    <div
      ...
      data-is-ready={!loading && !contentLoading && presentationData?.slides && presentationData.slides.length > 0}
    >
      {error ? (

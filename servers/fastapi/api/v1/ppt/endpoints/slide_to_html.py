...
async def generate_html_from_slide(
    ...
    """
    Generate HTML content from slide image and XML using Google Gemini 2.0 Flash.
    """
    user_config = get_user_config()
    api_key = user_config.GOOGLE_API_KEY
    
    if not api_key:
        raise HTTPException(status_code=400, detail="GOOGLE_API_KEY is not set in the configuration.")

    print(f"Generating HTML from slide using Google Gemini 2.0 Flash...")
    ...
    try:
        ...
        image_blob = base64.b64decode(base64_image)
        image_part = {"mime_type": media_type, "data": image_blob}
        ...
        if "API key" in str(e):
             raise HTTPException(status_code=401, detail=f"Invalid API Key for Google Gemini: {str(e)}")
...
async def generate_react_component_from_html(
    ...
    """
    Convert HTML content to TSX React component using Google Gemini 2.0 Flash.
    """
    user_config = get_user_config()
    api_key = user_config.GOOGLE_API_KEY
    ...
    print(f"Generating React component from HTML using Google Gemini 2.0 Flash...")

    try:
        genai.configure(api_key=api_key)
        model = genai.GenerativeModel('models/gemini-2.0-flash')
        
        # Construct the request for Google API
        google_request_parts = [HTML_TO_REACT_SYSTEM_PROMPT]
        google_request_parts.append(f"HTML INPUT:\\n{html_content}")
        
        if image_base64 and media_type:
            image_blob = base64.b64decode(image_base64)
            image_part = {"mime_type": media_type, "data": image_blob}
            google_request_parts.append(image_part)
        
        response = await model.generate_content_async(google_request_parts)
        ...
        if not react_content:
            raise HTTPException(
                status_code=500, detail="No React component generated"
            )
        ...
        if "timeout" in error_msg.lower():
            raise HTTPException(
                status_code=408,
                detail=f"Google Gemini API timeout during React generation: {error_msg}",
            )
        elif "connection" in error_msg.lower():
            raise HTTPException(
                status_code=503,
                detail=f"Google Gemini API connection error during React generation: {error_msg}",
            )
        else:
            raise HTTPException(
                status_code=500,
                detail=f"Google Gemini API error during React generation: {error_msg}",
            )
...
async def edit_html_with_images(
    ...
    """
    Edit HTML content based on one or two images and a text prompt using Google Gemini 2.0 Flash.
    ...
    try:
        ...
        print("Making Gemini API request for HTML editing...")

        # Construct request parts
        request_parts = [HTML_EDIT_SYSTEM_PROMPT]
        
        # Add current UI image
        current_image_blob = base64.b64decode(current_ui_base64)
        current_image_part = {"mime_type": media_type, "data": current_image_blob}
        request_parts.append(current_image_part)
        
        # Add sketch image if provided
        if sketch_base64:
            sketch_image_blob = base64.b64decode(sketch_base64)
            sketch_image_part = {"mime_type": media_type, "data": sketch_image_blob}
            request_parts.append(sketch_image_part)
        
        # Add text content
        text_content = f"CURRENT HTML TO EDIT:\n{html_content}\n\nTEXT PROMPT FOR CHANGES:\n{prompt}"
        request_parts.append(text_content)
        ...
        if not edited_html:
            raise HTTPException(
                status_code=500,
                detail="No edited HTML content generated by Google Gemini",
            )
        ...
        if "timeout" in error_msg.lower():
            raise HTTPException(
                status_code=408,
                detail=f"Google Gemini API timeout during HTML editing: {error_msg}",
            )
        elif "connection" in error_msg.lower():
            raise HTTPException(
                status_code=503,
                detail=f"Google Gemini API connection error during HTML editing: {error_msg}",
            )
        else:
            raise HTTPException(
                status_code=500,
                detail=f"Google Gemini API error during HTML editing: {error_msg}",
            )
...
async def convert_slide_to_html(request: SlideToHtmlRequest):
    """
    Convert a slide image and its OXML data to HTML using Google Gemini 2.0 Flash.
    """
    try:
        ...
        media_type = mimetypes.guess_type(actual_image_path)[0] or "image/png"
...
async def edit_html_with_images_endpoint(
    ...
    """
    Edit HTML content based on one or two uploaded images and a text prompt using Google Gemini 2.0 Flash.
